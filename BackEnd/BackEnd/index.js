...
app.post("/seller/add/product/:sellerId",auth,async (req,res)=>{
    ...
    try {
        ...
        await client.db("ecommerce").collection("seller").updateOne({_id: Obj_id}, {$set: {products: seller.products}});
...
app.get("/seller/products/:sellerId",auth,async (req,res)=>{
    try {
        ...
        const seller = await client.db("ecommerce").collection("seller").findOne({_id: new ObjectId(sellerId)});
...
app.get("/all/products",async (req,res)=>{
    try {
        const products = await client.db("ecommerce").collection("seller").aggregate([
            { $unwind: "$products" },
            { $project: {
                _id: 0,
...
app.post("/checkout/:userId",auth,async (req,res)=>{
    try {
        ...
        for (const item of req.body.cartItems) {
            ...
            const user = await client.db("ecommerce").collection("user").findOne({ _id: new ObjectId(userId) });

            if(user){
                const totalPurchases = (user?.purchaseCount || 0) + quantity; // Increment purchase count
                await client.db("ecommerce").collection("user").updateOne(
            ...
            const seller = await client.db("ecommerce").collection("seller").findOne({ _id: new ObjectId(seller_id) });
            if(seller){ 
                const totalSells = (seller?.sellCount || 0) + quantity; // Increment sell count
                await client.db("ecommerce").collection("seller").updateOne(
...
app.post("/best/seller/products",async (req,res)=>{
    try {
        const sellers = await client.db("ecommerce").collection("seller").find({}).toArray();
        if(sellers){    
...
app.post("/seller/add/banner/:sellerId",auth,async (req,res)=>{
    try {
        ...
        if (seller) {
            await client.db("ecommerce").collection("banner").insertOne({ sellerId: sellerId, sellerName: seller.name, ...bannerData });
...
app.get("/seller/get/banner",async (req,res)=>{
    try {
        const banner = await client.db("ecommerce").collection("banner").find({}, { projection: { _id: 0 } }).toArray();
        res.send(banner);
    } catch (error) {
...
app.delete("/seller/delete/product/:sellerId/:productId",auth,async (req,res)=>{
    try {
        ...
        await client.db("ecommerce").collection("seller").updateOne({ _id: new ObjectId(sellerId) }, { $set: { products: seller.products } });
...
app.get("/seller/banner/:sellerId",async (req,res)=>{
    try {
        const banners = await client.db("ecommerce").collection("banner").find({ sellerId: req.params.sellerId }).toArray();
        res.send(banners);
    } catch (error) {
        res.status(500).json({ message: 'Error fetching banner', error });
    }
...
app.delete("/seller/delete/banner/:sellerId/:bannerId",auth,async (req,res)=>{
    try {
        ...
        const banner = await client.db("ecommerce").collection("banner").findOne({ _id: new ObjectId(bannerId) });
        if (!banner) {
            return res.status(404).json({ message: 'Banner not found' });
        }

        await client.db("ecommerce").collection("banner").deleteOne({ _id: new ObjectId(bannerId) });
